...
async def stopit(event, flags):
    ...
    await event.reply("Will try to stop them, please wait...")
...
async def revive(event):
    ...
    if not (
        ...
        return
    await a.edit("Revert request sent. This might take 10minutes or so.")
...
Commands:
    `stopit` - Reply to a message WITH reason to send a request to Inspectors for judgement
    `approve` - Approve a scan request (Only works in System Base)
